...
class Viterbi(object):
    ...
    def decode(self, log_frame_probs):
        ...
        frame_scores = np.cumsum(log_frame_probs,
                                 axis=0)  # cumulative frame scores allow for quick lookup if frame_sampling > 1
    ...
    def decode_frame(self, t, old_hyp, frame_scores):
        ...
        for key, hyp in old_hyp.items():
            ...
            new_key = context + (label, min(length + self.frame_sampling, self.frame_sampling))
            # new_key = context + (label, min(length + self.frame_sampling, self.length_model.max_length()))
            # new_key = context + (label, min(length + self.frame_sampling))
            ...
            for new_label in self.grammar.possible_successors(context):
                ...
                score = hyp.score + self.frame_score(frame_scores, t, label) + self.grammar.score(context,
                                                                                                  new_label) + self.length_model.score(
                    length, label);  #
    ...
    def finalize_decoding(self, old_hyp):
        ...
        for key, hyp in old_hyp.items():
            context, label, length = key[0:-2], key[-2], key[-1]
            context = context + (label,)
            score = hyp.score + self.grammar.score(context, self.grammar.end_symbol()) + self.length_model.score(length,
                                                                                                                 label);  #
